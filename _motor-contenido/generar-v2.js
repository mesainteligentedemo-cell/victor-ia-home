#!/usr/bin/env node
/*
 * MOTOR DE CONTENIDO DIARIO — Victor IA Blog v2.0
 * Genera artículos y noticias con SEO + AEO + schema + link building + SVG temático + voz.
 *
 * USO:
 *   node generar-v2.js articulo  <archivo-contenido.json>
 *   node generar-v2.js noticia   <archivo-contenido.json>
 *
 * El JSON de contenido define: slug, title, desc, keywords, cover, readTime, category,
 * resumen, faqs, body. El motor genera: meta tags, Article+FAQPage+Breadcrumb schema,
 * SVG animado por categoría, bloque "En resumen", link building, reader de voz (article-reader.js),
 * estilos premium (blog-premium.css/js), y lo registra en sitemap + índice.
 */
const fs = require("fs");
...
// ── Mapeo de categorías a SVGs ─────────────────────────────────────────────
const CATEGORY_TO_SVG = {
  arquitectura: "/public/assets/svg/svg-arquitectura.svg",
  ventas: "/public/assets/svg/svg-ventas.svg",
  finanzas: "/public/assets/svg/svg-finanzas.svg",
  operaciones: "/public/assets/svg/svg-operaciones.svg",
  seguridad: "/public/assets/svg/svg-seguridad.svg",
  industria: "/public/assets/svg/svg-industria.svg",
  automatización: "/public/assets/svg/svg-automatización.svg",
  transformación: "/public/assets/svg/svg-transformación.svg",
  vtc: "/public/assets/svg/svg-vtc.svg",
  analítica: "/public/assets/svg/svg-analítica.svg",
};
...
// ── Generar key-takeaways automáticos si no existen ─────────────────────────
function generateKeyTakeaways(body) {
  // Extrae los primeros 5 párrafos y toma los primeros 5 "frases fuertes"
  const paras = body.replace(/<[^>]+>/g, " ").split(/\./).filter(p => p.trim().length > 20).slice(0, 5);
  if (paras.length === 0) return [];
  return paras.map(p => {
    const trimmed = p.trim();
    if (trimmed.length > 150) return trimmed.substring(0, 147) + "...";
    return trimmed + ".";
  });
}

// ── SVG por categoría (incluído inline o referenciado) ────────────────────────
function buildSvgSection(category) {
  // Por ahora, si la categoría existe en el mapeo, incluye una referencia.
  // En fase posterior, incorporaremos los SVGs generados.
  const svgPath = CATEGORY_TO_SVG[category?.toLowerCase()] || CATEGORY_TO_SVG.arquitectura;

  // Nota: esto es un placeholder. Los SVGs reales serán generados en FASE 2.
  // Por ahora, retorna un bloque visible que indica qué categoría debería usarse.
  return `
  <aside id="category-diagram" class="rv" role="img" aria-label="Diagrama relacionado con ${category || 'arquitectura'}" style="margin:10px 0 40px;border:1px solid rgba(184,154,106,.2);border-radius:14px;padding:22px 14px;background:radial-gradient(120% 120% at 50% 0%,rgba(184,154,106,.06),rgba(7,7,8,0) 70%)">
    <div style="text-align:center;color:rgba(234,230,223,.4);font-size:12px;padding:20px">
      [SVG de categoría: ${category || 'arquitectura'} — será incorporado en fase 2]
    </div>
  </aside>`;
}
...
.viai-related{border-top:1px solid rgba(184,154,106,.16);padding-top:32px;margin-top:44px}
.viai-related h2{font-family:'Cormorant Garamond',serif;font-size:24px;font-weight:300;margin-bottom:18px}
.related-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:18px}
.related-card{text-decoration:none;border:1px solid rgba(255,255,255,.07);border-radius:10px;padding:16px;display:block;transition:all .3s}
.related-card:hover{border-color:rgba(184,154,106,.5);background:rgba(184,154,106,.03)}
.related-card .cat{font-size:10.5px;letter-spacing:.12em;color:#B89A6A;text-transform:uppercase;display:block}
.related-card .ttl{display:block;margin-top:8px;color:rgba(234,230,223,.85);font-size:14.5px;line-height:1.4}
...
@media(max-width:640px){.lb-grid{grid-template-columns:1fr;gap:26px}.related-grid{grid-template-columns:1fr}}
</style>
</head>
<body>
...
// ── Artículos relacionados por categoría (stub para esta fase) ──────────────
function relatedArticles(slug, category) {
  // En fase posterior, esto leerá el índice de todos los JSONs en _motor-contenido/
  // Por ahora, retorna un bloque placeholder.
  return `
  <section class="viai-related rv">
    <h2>Artículos relacionados en ${esc(category || "su categoría")}</h2>
    <div class="related-grid">
      <p style="color:rgba(234,230,223,.4);font-size:12px;grid-column:1/-1">[Artículos relacionados serán llenados en fase de indexación]</p>
    </div>
  </section>`;
}
...
function buildArticulo(c) {
  // Auto-generar key-takeaways si no existen
  const resumen = c.resumen && c.resumen.length > 0 ? c.resumen : generateKeyTakeaways(c.body);
  ...
  const resumoBloque = resumen && resumen.length ? `<div class="key-takeaways rv"><h4>Lo más importante</h4><ul>\n${resumen.map(p => `<li>${esc(p)}</li>`).join("\n")}\n</ul></div>` : "";
  ...
  const svg = buildSvgSection(c.category);
  const body = `<div class="prose rv">\n${c.body}\n</div>`;
  const related = relatedArticles(c.slug, c.category);

  return metaHead(c, "articulo") + img + meta + h1 + resumoBloque + svg + body + related + linkBuilding() + footer();
}
...
function registrar(c, type) {
  ...
  if (!idx.includes(`href="${c.slug}.html"`)) {
    const tag = type === "noticia" ? "Noticias" : (c.category || "Estrategia");
...
if (!mode || !file) { console.error("USO: node generar-v2.js [articulo|noticia] contenido.json [--date=YYYY-MM-DD]"); process.exit(1); }
...
console.log(`  → SVG de categoría: ${c.category || 'arquitectura'}`);
console.log(`  → blog-premium.css/js + article-reader.js incluidos`);
console.log(`  → key-takeaways: ${(c.resumen && c.resumen.length) || generateKeyTakeaways(c.body).length} puntos`);
console.log(`  → registrado en sitemap + índice del blog`);
console.log(`  → FALTA: generar portada + deploy`);